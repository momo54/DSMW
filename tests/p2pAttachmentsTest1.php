...
[[Category:Animal]]";
	...
	public function testSimple1() {
		...
		foreach ($PatchonWiki1 as $key => $value) {
			echo $value; 
		}
		
		echo "patchonWiki1 = " . count($PatchonWiki1) . "\n";
		
		$PatchonWiki1 = arraytolower($PatchonWiki1);
		$this->assertEquals(1, count($PatchonWiki1), count($PatchonWiki1) . ' should be equal to 1');
		...
		$PatchonWiki1 = getSemanticRequest($this->p2pBot1->bot->wikiServer, '[[Patch:+]][[onPage::'.$this->filePage.']]', '-3FpatchID');
		$PatchonWiki1 = arraytolower($PatchonWiki1);
		$this->assertEquals(2, count($PatchonWiki1), 'count($PatchonWiki1) should be equal to 2');
