        ...
        // Now we need to check whether our edit was accepted. If it was, we'll get a 302 redirecting us to the article. If it wasn't (e.g. because of an edit conflict), we'll get a 200.
            ...
            if ('200'==$code) {
                echo "updated properties failed with error 200:(".$this->bot->results.")\n"
                      .$this->bot->wikiServer.PREFIX.'/extensions/DSMW/bot/DSMWBot.php'."\n";
                return false;
            }
            elseif ('302'==$code)
                return true;
...
//        $editToken = substr($this->bot->results, $val+$val2, $val1-($val+$val2));
        ...
        ///////MW>1.16.0////////
        $text = $this->bot->results;
        
		preg_match('<input type="hidden" value="(.*)" name="editToken" />',$text,$res);
		$editToken = $res[1];
        /*$val = strpos($text, '" name="editToken"');
        $eval = $val;
        $startsearchval = $val-50;
        $val1 = strpos($text, 'value="', $startsearchval);
        $length = $eval - ($val1+7);
        $editToken = substr($text, $val1+7, $length);*/
        ///////MW>1.16.0////////
        $fp = fopen($file, 'r');
...
//        $post_vars['xmlimport']['type']= 'text/xml';
    ...
    function getFileFeatures($file,$size){    
    	//echo "    getFileFeatures($file,$size)";
        ...
        $array = array_shift($array);
        /*$desc = $doc->query->pages->page->imageinfo->ii;
        if (isset($desc['size']))
