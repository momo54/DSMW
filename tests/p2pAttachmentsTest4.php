...
/**
 * Description of p2pAttachmentTest4
 *
 * @author Émile Morel
 */


class p2pAttachmentsTest4 extends PHPUnit_Framework_TestCase {
    ...
    var $pageName1 = "Mammifère";
    var $pushRequest1 = "[[Category:Mammal]]";
    var $pushFeed1 = 'PushFeed:PushMammal';
    var $pullFeed1 = 'PullFeed:PullMammal';
    var $pushName1 = 'PushMammal';
    var $pullName1 = 'PullMammal';
...
[[Category:Animal]]";
    var $content1="Les Mammifères (classe des Mammalia) forment un taxon inclus dans
    les vertébrés, traditionnellement une classe , définie dès la classification de Linné.
    Ce taxon est considéré comme monophylétique.
[[Image:ours.jpg|right|frame|Un Ours]]
[[Category:Mammal]]";
    ...
    public function testSimple1() {
        ...
        //create second page
        $this->assertTrue($this->p2pBot1->createPage($this->pageName1,$this->content1),
                'Failed to create page '.$this->pageName1.' ('.$this->p2pBot1->bot->results.')');

        // assert page Ours exist on wiki1
        assertPageExist($this->p2pBot1->bot->wikiServer, $this->pageName1);
        ...
        //edit File:Ours.jpg on wiki1
        $this->assertTrue($this->p2pBot1->editPage($this->filePage, $this->pushRequest1),
            'failed to edit page '.$this->filePage.' ( '.$this->p2pBot1->bot->results.' )');
        ...
        //create push on wiki1
        $this->assertTrue($this->p2pBot1->createPush($this->pushName1, $this->pushRequest1),
                'Failed to create push : '.$this->pushName1.' ('.$this->p2pBot1->bot->results.')');

        //push
        $this->assertTrue($this->p2pBot1->push($this->pushFeed1),
                'failed to push '.$this->pushFeed1.' ('.$this->p2pBot2->bot->results.')');
        ...
        $this->assertTrue($this->p2pBot3->createPull($this->pullName1,$this->wiki1, $this->pushName1),
                'failed to create pull '.$this->pullName1.' ('.$this->p2pBot3->bot->results.')');

        //pull
        $this->assertTrue($this->p2pBot3->Pull($this->pullFeed1),
                'failed to pull '.$this->pullFeed1.' ('.$this->p2pBot3->bot->results.')');
        ...
        $this->assertTrue($this->p2pBot3->createPush($this->pushName1, $this->pushRequest1),
                'Failed to create push : '.$this->pushName1.' ('.$this->p2pBot3->bot->results.')');

        //push 
        $this->assertTrue($this->p2pBot3->push($this->pushFeed1),
                'failed to push '.$this->pushFeed1.' ('.$this->p2pBot3->bot->results.')');




        //create pull on wiki1 from wiki3
        $this->assertTrue($this->p2pBot1->createPull($this->pullName1,$this->wiki3, $this->pushName1),
                'failed to create pull '.$this->pullName1.' ('.$this->p2pBot1->bot->results.')');

        //pull
        $this->assertTrue($this->p2pBot1->Pull($this->pullFeed1),
                'failed to pull '.$this->pullFeed1.' ('.$this->p2pBot1->bot->results.')');
        ...
        $this->assertEquals(count($PatchonWiki1),5);
