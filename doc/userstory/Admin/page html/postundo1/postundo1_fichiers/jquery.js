...
(function($) {
    ...
    $.ajaxSetup({
        stream: false,
        pollInterval: 500/*, onDataRecieved: function(){}*/
    });
    ...
    var ajax_stream = $.ajax = function(options)
    {
        ...
        if(options.stream)
        {
            ...
            var fix = function(){
                check('stream');
            };// fixes weird bug with random numbers as arg
            var checkagain = function(){
                if(!done) timer = setTimeout(fix,options.pollInterval);
            }
            ...
            if($.isFunction(options.success))
            {
                ...
                options.success = function(xhr,s){
                    complete(xhr,s);
                    oc(xhr,s);
                };
